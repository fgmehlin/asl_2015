...
public class InboxProcessingThread implements Runnable{
	...
	InboxProcessingThread(int mwID, int nbC, BlockingQueue<QueryObject> in, BlockingQueue<QueryObject> out, ConnectionPoolManager poolManager, int proc_id){
		this.in = in;
		this.out = out;
		this.mwID = mwID;
		this.nbC = nbC;
		...
		while(true){
			...
			try {
				...
				if(!cmd.equals("ECHO")){
					...
					query.setReply("99999999");
				}else{
					query.setReply(MiddleWare.clientIndex+"");
					MiddleWare.clientIndex++;
				}
