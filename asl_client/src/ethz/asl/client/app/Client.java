...
public class Client {
	...
	public static void sendMessage(PrintWriter out, BufferedReader in) throws IOException {
		...
		if (queue > 0) {
			if (receiver > 0) {
				// long id =
				// (clientID+System.currentTimeMillis())+"".hashCode();
				// logger.info("Client "+ clientID + " Q send message to
				// client(" + receiver + ") to queue("+queue+")");
				...
				// logger.info("Client "+ clientID + " R send message to
				// client(" + receiver + ") to queue("+queue+"). Status : "
				// +sendStatus);
			} else {
				// logger.info("No client to send a message for client: " +
				// clientID);
				// System.out.println("No client available");
			}
		} else {
			// logger.info("No queue to send a message for client: " +
			// clientID);
	...
	public static void peekMessageByQueue(PrintWriter out, BufferedReader in) throws IOException {
		...
		// logger.info("Client "+ clientID + " Q peek message by
		// queue("+queue+")");
		startQuery = System.currentTimeMillis();
		...
		if (messagePeeked == null || messagePeeked.equals("null")) {
			...
			// logger.info("Client "+ clientID + " R peek message by
			// queue("+queue+"). NO MSG AVAILABLE");
		} else {
			logger.info("[RESPONSE][PMQ] " + responseTime + " [" + messagePeeked + "]");
			// logger.info("Client "+ clientID + " R peek message by
			// queue("+queue+"). Message="+messagePeeked);
		}
		// System.out.println("Message peeked from Queue "+queue+" :" +
		// messagePeeked);
	}
	...
	public static void peekMessageBySender(PrintWriter out, BufferedReader in) throws IOException {
		...
		if (messagePeeked == null || messagePeeked.equals("null")) {
			...
			// logger.info("Client "+ clientID + " R peek message by
			// sender("+sender+"). NO MSG AVAILABLE");
		} else {
			logger.info("[RESPONSE][PMS] " + responseTime + " [" + messagePeeked + "]");
			// logger.info("Client "+ clientID + " R peek message by
			// sender("+sender+"). Message="+messagePeeked);
		}

		// System.out.println("Message peeked from Sender "+client+" :" +
		// messagePeeked);
	}

	public static void popMessageByQueue(PrintWriter out, BufferedReader in) throws IOException {
		int queue = getQueueWithMSG(out, in);
		// logger.info("Client "+ clientID + " Q pop message by
		// queue("+queue+")");
		...
		if (messageFromQueue == null || messageFromQueue.equals("null")) {
			...
			// logger.info("Client "+ clientID + " R pop message by
			// sender("+queue+"). NO MSG AVAILABLE");
		} else {
			logger.info("[RESPONSE][GMQ] " + responseTime + " [" + messageFromQueue + "]");
			// logger.info("Client "+ clientID + " R pop message by
			// sender("+queue+"). Message="+messageFromQueue);
		}

		// System.out.println("Message poped from Queue "+queue+" :" +
		// messageFromQueue);
	}
	...
	public static void popMessageBySender(PrintWriter out, BufferedReader in) throws IOException {
		...
		if (messageFromSender == null || messageFromSender.equals("null")) {
			...
			// logger.info("Client "+ clientID + " R pop message by
			// sender("+sender+"). Message="+messageFromSender);
		}

		// System.out.println("Message peeked from Sender "+sender+" :" +
		// messageFromSender);
	...
	public static void createQueue(PrintWriter out, BufferedReader in) throws IOException {
		...
		// logger.info("Client "+ clientID + " R create a queue. Queue ID : " +
		// queueid);
	}
	...
	public static void deleteQueue(PrintWriter out, BufferedReader in) throws IOException {
		...
		if (queueid >= 0) {
			...
			// logger.info("Client "+ clientID +" R delete queue No"+queue+"
			// status="+response);
		} else {
	...
	private static int getClient(PrintWriter out, BufferedReader in) throws IOException {
		...
		if (!clients.isEmpty()) {
			// logger.info("Client " + clientID + " R list of clients. List size
			// : " + clientList.length);
	...
	private static int getClientWithMSG(PrintWriter out, BufferedReader in) throws IOException {
		// logger.info("Client " + clientID + " Q list of clients with
		// messages.");
		...
		if (!clients.isEmpty()) {
			// logger.info("Client " + clientID + " R list of clients with
			// messages. List size : " + clientList.length);
			int receiver_index = rand.nextInt(clientList.length);
			return Integer.parseInt(clientList[receiver_index]);
		} else {
			// logger.info("Client " + clientID + " R list of clients with
			// messages. List size : 0");
			return -1;
		}
	}
	...
	private static int getQueue(PrintWriter out, BufferedReader in) throws IOException {
		...
		if (!queues.isEmpty()) {
			// logger.info("Client " + clientID + " R list of queues. List size
			// : " + queueList.length);
	...
	private static int getQueueWithMSG(PrintWriter out, BufferedReader in) throws IOException {
		...
		if (!queues.isEmpty()) {
			// logger.info("Client " + clientID + " Q list of queues with
			// messages. List size : " + queueList.length);
			int queue_index = rand.nextInt(queueList.length);
			return Integer.parseInt(queueList[queue_index]);
		} else {
			// logger.info("Client " + clientID + " R list of queues with
			// messages. List size : 0");
			return -1;
		}
	}
